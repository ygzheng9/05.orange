 ...
 */
layui.define(["element", "laytpl", "jquery", "metis"], function (exports) {
    var element = layui.element,
        $ = layui.$,
        laytpl = layui.laytpl,
        layer = layui.layer;

    const metis = layui.metis;
    ...
    var miniMenu = {
        ...
        render: function (options) {
            ...
            if (options.multiModule) {
                miniMenu.renderMultiModule(
                    options.menuList,
                    options.menuChildOpen
                );
            } else {
                miniMenu.renderSingleModule(
                    options.menuList,
                    options.menuChildOpen
                );
            }
        ...
        compileMenu: function (menu, isSub) {
            var menuHtml =
                '<li {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} class="layui-nav-item menu-li {{d.className}}"  {{#if( d.id){ }}  id="{{d.id}}" {{#}}}> <a {{#if( d.href){ }} layuimini-href="{{d.href}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}} href="javascript:;">{{#if( d.icon){ }}  <i class="{{d.icon}}"></i> {{#}}} <span class="layui-left-nav">{{d.title}}</span></a>  {{# if(d.children){}} {{d.children}} {{#}}} </li>';
            if (isSub) {
                menuHtml =
                    '<dd class="menu-dd  {{#if( d.className){ }} {{d.className }} {{#}}}"> <a href="javascript:;"  {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} {{#if( d.id){ }}  id="{{d.id}}" {{#}}} {{#if(( !d.child || !d.child.length ) && d.href){ }} layuimini-href="{{d.href}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}}> {{#if( d.icon){ }}  <i class="{{d.icon}}"></i> {{#}}} <span class="layui-left-nav"> {{d.title}}</span></a> {{# if(d.children){}} {{d.children}} {{#}}}</dd>';
            }
            return laytpl(menuHtml).render(menu);
        },
        compileMenuContainer: function (menu, isSub) {
            var wrapperHtml =
                '<ul class="layui-nav layui-nav-tree layui-left-nav-tree {{d.className}}" id="{{d.id}}">{{d.children}}</ul>';
            if (isSub) {
                wrapperHtml =
        ...
        renderLeftMenu: function (leftMenus, options) {
            options = options || {};
            var me = this;
            var leftMenusHtml = me
                .each(leftMenus || [], function (idx, leftMenu) {
                    // 左侧菜单遍历
        ...
        renderMultiModule: function (menuList, menuChildOpen) {
            ...
            if (menuChildOpen) childOpenClass = " layui-nav-itemed";
            var headerMenuHtml = this.each(menuList, function (index, val) {
                //顶部菜单渲染
        ...
        listen: function () {
            ...
            $("body").on(
                "click",
                ".layuimini-header-menu.layuimini-mobile-show dd",
                function () {
                    var loading = layer.load(0, {
                        shade: false,
                        time: 2 * 1000,
                    });
                    var check = $(".layuimini-tool [data-side-fold]").attr(
                        "data-side-fold"
                    );
                    if (check === "1") {
